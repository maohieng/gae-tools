...
public abstract class BaseGaeSearchService<T, K> implements SearchExecutor<T, K, SearchImpl<T, K>> {
	...
	protected BaseGaeSearchService(Class<T> type, SearchConfig searchConfig, IndexNamingStrategy indexNamingStrategy) {
		this(type, new SearchMetadata<T, K>(type, searchConfig.getIndexTypeLookup()), searchConfig, indexNamingStrategy);
	}
	...
	protected BaseGaeSearchService(Class<T> type, Class<K> keyType, SearchConfig searchConfig, IndexNamingStrategy indexNamingStrategy) {
		this(type, new SearchMetadata<T, K>(type, keyType, searchConfig.getIndexTypeLookup()), searchConfig, indexNamingStrategy);
	}
	...
	protected SearchImpl<T, K> search() {
		return new SearchImpl<T, K>(this);
	}
	...
	public static class Transformers {
		public static CollectionTransformer<ScoredDocument, String> ToIds = Expressive.Transformers.transformAllUsing(new ETransformer<ScoredDocument, String>() {
			@Override
